...
with open(LOG_INPUT, "r") as f:
    for line in f:
        try:
            ...
            log_time = log["time"]
            # NOTE: Tidak parse log_time lagi, langsung pakai string
            if last_time:
                # Karena format log["time"] tidak bisa diparse, abaikan pengecekan berdasarkan waktu log
                pass

            message = log.get("message", "")
            base_entry = {
                "reqId": req_id,
                "time": log_time,  # Gunakan langsung dari log["time"]
